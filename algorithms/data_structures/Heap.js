...
class MinHeap {
  ...
  shiftDown() {
    ...
    while (this.idxOfLeftChild < heap.length) {
      const idxOfRightChild = this.idxOfLeftChild + 1;
      let idxOfSmallestChild = this.idxOfLeftChild;

      if (
        idxOfRightChild < heap.length &&
        heap[idxOfRightChild] < heap[this.idxOfLeftChild]
      ) {
